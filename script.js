window.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("bookmarkletBtn").addEventListener("click", () => {
      const bookmarkletCode = "javascript:(function(){ window.name = document.documentElement.outerHTML; window.location ='http://view.tasnimhasan.site/'; })();";
      prompt("Right-click and copy the following bookmarklet code:", bookmarkletCode);
    });
});
...
function extractVideoLinks(htmlSource) {
    ...
    if (videos.length === 0) {
      showMessage("No video links found.");
